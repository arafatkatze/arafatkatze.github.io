/* Guestbook — text sibling of /pixels. Persists to pantry.cloud. */
(function () {
  'use strict';
  ...
  function fetchEntries() {
    return fetch(PANTRY_URL, { headers: { Accept: 'application/json' } })
      ...
      .then(function (data) { return (data && Array.isArray(data.entries)) ? data.entries : []; })
  ...
  fetchEntries().then(function (entries) {
    if (entries === null) { setMeta("couldn't reach the guestbook — try again in a bit"); return; }
    render(entries);
  });
  ...
  formEl.addEventListener('submit', function (ev) {
    ...
    .finally(function () { if (btn) { btn.disabled = false; btn.textContent = 'Sign \u00a0\u2192'; } });
  });
})();
